						 ...
						 if (!fs.existsSync(uploadPath)) {
								mkdirp(uploadPath, (err) => {
									if (err) {
										...
										processing.processResult(ffOrderNumber, PullSeqNo, dataPullName, trailName, years, (success) => {
											if (success == true) {
												callGD();
											}
										});
        ...
        try {
            function getjsonfiles(auth, dataPullFolderId, callback) {
                ...
                service.files.list({
                    auth,
                    fields: 'nextPageToken, files(id, name)',
                        q: "'" + dataPullFolderId1 + "'" + " in parents and trashed=false  and  (name contains 'profileinfo' or name contains 'OutStandingDemand' or name contains 'DataPullSummary' ) and mimeType contains 'json'", //quering the google drive with parameter
                }, (error, response) => {
                    if (error) {
                        // return console.log("ERROR", error);
                        errorRecovery.errRecovery(ffOrderNumber, 1, error);
                        ...
                        if (response.files.length > 0) {
                            // checking the folder length
                            if (response.files.length > 3) {
                                processing.AppendFile(ffOrderNumber, 'Error  contains more than  3 json files');
                                    ...
                                    while (j < gdYears.length) {
                                        if (years[i] == gdYears[j].name) {
                                            getFile5(auth, gdYears[j].id, gdYears[j].name, (success) => {
                                                if (success == true) {
                                                    k++;
                                                    console.log(k, response.files.length);
                                                    if (k == response.files.length) {
                                                        if (success == true) {
                        ...
                        if (response.files.length > 0) {
                            ...
                            if (response.files.length > 1) {
                                ...
                                getFile8A(auth, dateOneId, orgRevFoldername2, yearName4, (success) => {
                                    if (success == true) {
                                        getpdffiles(auth, response.files[0].id, (success1) => {
                                            if (success1 == true) {
                                                console.log('3');
                                                return callback(true);
                                            } else {
						   ...
						   processing.updateProceFile(ffOrderNumber,20,yearName4,'Failure-There is no date folder ');
                            return callback(true);
                        }
                    }
                });
                                    ...
                                    response.files.forEach((item) => {
                                        moveToDest(auth, item.id, item.name, destId, (success) => {
                                            ...
                                            if (success == true) {
												...
												processing.updateProceFile(ffOrderNumber,21,yearName,'Successfuly Files Moved');
                                                    return callback(true);
                                                }
                                            } else {
                                                return callback(false);
                    ...
                    if (error) {
                        ...
                        if (response.files.length == 0) {
                           ...
                           // processing.AppendFile(ffOrderNumber, 'No Ak no folder');
						     processing.updateProceFile(ffOrderNumber,20,yearName5,'Failure-No Ak no folder');
                            return callback(true);
        ...
        try {
            function getFile8(auth, AkFolderId, orgRevFoldername3, yearName5, callback) {
                ...
                service.files.list({
                    auth,
                    fields: 'nextPageToken, files(id, name)',
                        q: "'" + AkFolderId1 + "'" + " in parents and trashed=false and mimeType contains 'text/xml'", //quering the google drive with parameter
                }, (error, response) => {
                    if (error) {
                        return console.log('ERROR', error);
                    } else {
                        if (response.files.length == 0) {
                           ...
                           // processing.AppendFile(ffOrderNumber, 'No XML files in folder');
						    processing.updateProceFile(ffOrderNumber,20,yearName5,'Failure-No XML files in folder');
                            return callback(true);
                        } else if (response.files.length > 1) {
							console.log(response.files);
                            // console.log(" folder contain more then one XML file ")
                             processing.updateProceFile(ffOrderNumber,20,yearName5,'Failure-Multiple XML files in folder');
                            return callback(true);
                        } else {
