					...
					try{
						...
						// ITR-2   year 2013,2014,2015,2016,2017 && Commented mapping fields are not available for the particular file type
							...
							if(assyear[i] == 2017){
							   ...
							   // let amt;
								...
								try{
									var amtAll = select('//ITRForm:Amount/text()', doc);
									const AmtallDet = [];
									var amt =0;
									for (let i = 0; i < amtAll.length; i++) {
										AmtallDet[i] = amtAll[i].nodeValue;
										amt += parseFloat(AmtallDet[i]);

									}
									console.log(amt+'................................498');
								}
								catch(e){
									console.log('Amount not found  '+'................................474');
								}
						...
						// ITR-3  year 2013,2014,2015,2016,2017 && Commented mapping fields are not available for the particular file type
							...
							if(assyear[i] == 2016 ||  assyear[i] == 2015 || assyear[i] == 2014){
								...
								deduc10A[i] = checkFieldAvl('//ITRForm:DeductionsUnder10Aor10AA/text()', 0);
								// let amt1;
								   // amt1 = checkFieldAvl('//ITRForm:Amount/text()', 0);
								try{
									var amtAll = select('//ITRForm:Amount/text()', doc);
									const AmtallDet = [];
									var amt1 =0;
						...
						// ITR-4  year 2013,2014,2015,2016,2017  && Commented mapping fields are not available for the particular file type
                           ...
                           if(assyear[i] == 2016 ||  assyear[i] == 2015 || assyear[i] == 2014){
								...
								// Extracting Revenue from TotCreditsToPL or GrossReceipt
								...
								taxAtNor[i] = checkFieldAvl('//ITRForm:TotalTaxPayable/text()', 0);
								// let amt2;
								   // amt2 = checkFieldAvl('//ITRForm:Amount/text()', 0);
								   // console.log(amt2 +'..........');
								try{
									var amtAll = select('//ITRForm:Amount/text()', doc);
									const AmtallDet = [];
									...
									for (let i = 0; i < amtAll.length; i++) {
										AmtallDet[i] = amtAll[i].nodeValue;
										amt2 += parseFloat(AmtallDet[i]);

									}
									console.log(amt2+'................................498');
								}
								catch(e){
									console.log('Amount not found  '+'................................474');
								}
